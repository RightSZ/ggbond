...
aggrPlot<-function(d, col_replace=TRUE){
  if(col_replace){
    d[["missings"]][["Variable"]]<-str_replace_all(d[["missings"]][["Variable"]],"_"," ")
  }
  h<-d[["tabcomb"]][order(d[["count"]],decreasing = T),]
  colnames(h)<-d[["missings"]][["Variable"]]
  h <- melt(h)
  h$value<-factor(h$value)
  p1<-ggplot(h, aes(x = Var2, y = Var1, fill = value)) +
    geom_tile(colour="white") +  # 绘制每个单元格
  ...
  p<-data.frame(Col=d[["missings"]][["Variable"]],
                Proportion=d[["missings"]][["Count"]]/nrow(d[["x"]])
  )
  ...
  p2<-ggplot(p, aes(x = Col, y = Proportion))+
    ...
    theme_bw()+
    theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust = 0.5))
  l<-data.frame(percent=d[["percent"]][order(d[["count"]],decreasing = T)],
                count=d[["count"]][order(d[["count"]],decreasing = T)],
                color=ifelse(stringr::str_detect(d[["combinations"]][order(d[["count"]],decreasing = T)],"1"),"1","0"))
